...
struct DNSModel{Fl <: Real, Fβ <: Real, Fγ <: Real} <: AbstractDNSModel
    # Base
    base::KalmanBaseModel{Fl, Fβ, Fγ}

    # λ parameter
    lambda::Fl

    # Constructor
    function DNSModel{T}(maturities::Vector{T}, N::Int, M::Int, random_walk::Bool;
        ...
        base = KalmanBaseModel{T}(maturities, N, M, L, duplicator, random_walk,
    ...
    DNSModel{T}(base::KalmanBaseModel{T, Fβ, Fγ}, lambda::T) where {T<:Real, Fβ<:Real, Fγ<:Real} = new{T, Fβ, Fγ}(base, lambda)

end

function build(model::AbstractDNSModel, Z::Matrix{Fγ}, beta::Vector{Fβ}, gamma::Vector{Fγ}, Phi::Matrix{Fβ}, delta::Vector{Fβ}, mu::Vector{Fβ}, A::Vector{Fγ}, B::Vector{Fγ}, omega::Vector{Fγ}, nu::Vector{Fγ}) where {Fβ<:Real, Fγ<:Real}
